...
function HuipilStrap({ patternId }: { patternId: string }) {
  return (
    <svg aria-hidden="true" width="100%" height="24" style={{ display: "block" }}>
      <defs>
        <pattern id={patternId} x="0" y="0" width="60" height="24" patternUnits="userSpaceOnUse">
          ...
          <polygon points="30,8 46,12 30,16 14,12" fill="none" stroke="#D4A017" strokeWidth="1.2" />
...
export default function Marquee() {
  ...
  return (
    ...
    >
      ...
      <div className="que-marquee flex py-5 whitespace-nowrap sm:py-6">
        {loop.map((text, i) => (
          <div
            key={i}
          ...
          >
            <span className="font-serif text-2xl leading-none tracking-[0.01em] italic sm:text-3xl lg:text-4xl">
              {text}
            </span>
            <span
              aria-hidden="true"
              className="inline-block h-1.5 w-1.5 rounded-full bg-cream/60"
            />
          </div>
        ))}
      </div>
